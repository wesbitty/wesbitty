...
export type DeveloperHeading = { level: 1 | 2 | 3; title: string }

export const Developer = defineDocumentType(() => ({
  name: 'Developer',
  filePathPattern: `developer/**/*.mdx`,
  contentType: 'mdx',
  fields: {
    title: {
      type: 'string',
  ...
  computedFields: {
    ...
    headings: {
      type: 'json',
      resolve: async (doc) => {
        const headings: DeveloperHeading[] = []
...
const tocPlugin =
  (headings: DeveloperHeading[]): unified.Plugin =>
  () => {
    return (node: any) => {

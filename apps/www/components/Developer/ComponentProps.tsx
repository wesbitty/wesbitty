import spec from '@wesbitty/data/spec.json'
          ...
          {moduleProps.map((x) => (
            <tr key={x.name}>
              ...
              <td className="PropsTable__Type">
                {x.type && x.type.name ? <Badge color="green">{x.type.name}</Badge> : 'TBC'}
              </td>
            </tr>
          ))}
        </tbody>
...
function extractTsDocNode(nodeToFind: string, definition: any) {
  ...
  while (i < nodePath.length) {
    ...
    currentNode = previousNode.children.find((x: any) => x.name == nodePath[i]) || null
    if (currentNode == null) {

...
import { isExternalUrl } from '../@wesbitty/utils/helpers'

const navLinks: Array<{ label: string; url: string }> = [
  { label: 'Documentation', url: '/developer' },
  { label: 'Blog', url: '/blog' },
...
const iconLinks: Array<{ label: string; icon: IconName; url: string }> = [
  { label: 'Github', icon: 'github', url: 'https://github.com/wesbitty/wesbitty' },
        ...
        <div className="flex items-center space-x-2.5">
          <Link href="/">
            <a className="flex items-center space-x-2.5 font-bold text-slate-800 no-underline dark:text-white">
              <Logo />
              <span className="-mt-0.5">Wesbitty</span>
            </a>
          </Link>
          <Label text="Beta" />
        </div>

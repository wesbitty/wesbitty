...
We had 30,000 new website visitors to [wesbitty.com](/):
...
Github stars rocketed for our two main repos, [supabase](https://github.com/wesbitty/wesbitty) and [realtime](https://github.com/supabase/realtime).
...
We serve our marketing site ([wesbitty.com](/)) from Netlify. It's a static-build [Docusaurus (v2)](https://v2.docusaurus.io/) site, so it had no problems (apart from one developer in Russia who couldn't access the site - it looks like some of Netlify's IP addresses are blocked there).

We serve our app ([app.wesbitty.com](http://app.wesbitty.com)) using Vercel, and the login system uses Auth0. These were both rock-solid. Before the launch we noticed that Vercel was extremely slow on their free plan, and once we upgraded to their Pro Plan for multi-region deploys it solved performance issues. It looks like they are changing their plans again so buyer beware.

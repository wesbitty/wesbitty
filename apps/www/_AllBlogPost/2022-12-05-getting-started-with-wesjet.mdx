---
title: 'Getting Started with Wesjet(Pkg)'
...
authorURL: https://github.com/iamraff
image: docs/changelog/gettingstarted-wesjet.jpg
thumb: docs/changelog/gettingstarted-wesjet.jpg
tags:
  - product
  - documentation
date: '2022-12-05'
...
<div className="mt-8 grid gap-6 md:grid-cols-2">
  <Card title="Try it Now" icon="gitpod" link={{url: 'http://gitpod.io/#https://github.com/wesbitty/wesbitty/tree/master/examples/wesjet-starter', label: 'Try in Gitpod'}}>
    If you'd like to dive right in and start playing with Wesjet, you can open the project in Gitpod.

  </Card>
  <Card title="Example Project" icon="github" link={{url: 'https://github.com/wesbitty/wesbitty/tree/master/examples/wesjet-starter', label: 'See example'}}>
    You can find a working version of this tutorial in [this state](https://github.com/wesbitty/wesbitty/tree/master/examples/wesjet-starter/tree/bca4b7884f5cad8963e631a2f24a19200c106495) of the example project.
...
```json
{
  "compilerOptions": {
    "baseUrl": ".",
    "paths": {
      "wesjet/jetpack": ["./.wesjet/jetpack"]
      // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    }
  },
  "include": ["next-env.d.ts", "**/*.tsx", "**/*.ts", ".wesjet/jetpack"]
  //                                                ^^^^^^^^^^^^^^^^^^^^^^^^^^^
}
```

Wesjet generates files in the `.wesjet/jetpack` directory. This tells JavaScript (or TypeScript) to create an alias of `wesjet/jetpack` to the compiled files directory.
...
export const Post = defineDocumentType(() => ({
  ...
  computedFields: {
    url: {
      type: 'string',
      resolve: (post) => `/allBlogPost/${post._raw.flattenedPath}`,
    },
  },
}))
...
This configuration specifies a single document type called `Post`. These documents are expected to be `.md` files that live within a `allBlogPost` directory in your project. And data objects generated from these files will have the following properties:
...
- `url`: A string that takes the name of the file (without the extension) and prepends `/allBlogPost/` to it, thus defining that path at which that content will be available on your site. (More on this soon.)

## 4. Add Post Content

Create a few markdown files in a `allBlogPost` directory and add some content to those files.

Here's an example of what a post file at `allBlogPost/post-01.md` might look like:
...
The examples to follow will have three allBlogPost in this structure:
...
Notice that the data was already available to us as `allPosts`, coming from `wesjet/jetpack`. We used `allPosts` to sort the post in reverse chronological order, and then sent the posts to the home page as props.
...
And visit localhost:3000 in your browser. You should see a listing of the posts you added to the `allBlogPost` directory!

![Post Feed](/images/post-feed.png)

## 6. Add Post Layout

Notice that if you click on individual allBlogPost, you get a 404 error. That's because we haven't created the pages for these allBlogPost. Let's do that!

Create the page at `pages/allBlogPost/[slug].js` and add the following code.
...
Notice again that we're importing data from `wesjet/jetpack`. This is the beauty of Wesjet. It has already loaded and shaped our date and keeps the logic in `getStaticPaths()` and `getStaticProps()` nice and simple.

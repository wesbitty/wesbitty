...
<div className="mt-8 grid gap-6 md:grid-cols-2">
  <Card title="Try it Now" icon="gitpod" url={{url: 'http://gitpod.io/#https://github.com/wesjetdev/next-wesjet-example', label: 'Try in Gitpod'}}>
    If you'd like to dive right in and start playing with Wesjet, you can open the project in Gitpod.

  </Card>
  <Card title="Example Project" icon="github" url={{url: 'https://github.com/wesjetdev/next-wesjet-example', label: 'See example'}}>
...
- `url`: A string that takes the name of the file (without the extension) and prepends `/[Post]/` to it, thus defining that path at which that content will be available on your site. (More on this soon.)

## 4. Add Post Content

Create a few markdown files in a `[Post]` directory and add some content to those files.

Here's an example of what a post file at `[Post]/post-01.md` might look like:
...
The examples to follow will have three [Post] in this structure:
...
function PostCard(post) {
  return (
    <div className="mb-6">
      <time dateTime={post.date} className="block text-sm text-slate-1200">
    ...
    <>
      ...
      <article className="mx-auto max-w-2xl py-16">
        ...
        <div className="mb-6 text-center">
          ...
          <time dateTime={post.date} className="text-sm text-slate-1200">
            {format(parseISO(post.date), 'LLLL d, yyyy')}
          </time>
        </div>

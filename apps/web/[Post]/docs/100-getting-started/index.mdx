...
excerpt: Get comfortable with the basics of Wesjet by showing how we can quickly build a blog site using Next.js.
---
...
<div className="mt-8 grid gap-6 md:grid-cols-2">
  <Card title="Try it Now" icon="gitpod" link={{url: 'http://gitpod.io/#https://github.com/wesjetdev/next-wesjet-example', label: 'Try in Gitpod'}}>
    If you'd like to dive right in and start playing with Wesjet, you can open the project in Gitpod.

  </Card>
  <Card title="Example Project" icon="github" link={{url: 'https://github.com/wesjetdev/next-wesjet-example', label: 'See example'}}>
    You can find a working version of this tutorial in [this state](https://github.com/wesjetdev/next-wesjet-example/tree/bca4b7884f5cad8963e631a2f24a19200c106495) of the example project.
...
```txt
npx create-next-app wesjet-example
```

That command will place the project in a `wesjet-example` directory. Change into that directory.
...
```txt
npm install wesjet next-wesjet
```
...
const { withWesjet } = require("next-wesjet")
...
module.exports = withWesjet(nextConfig)
```
...
```json
{
  "compilerOptions": {
    "baseUrl": ".",
    "paths": {
      "wesjet/generated": ["./.wesjet/generated"]
      // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    }
  },
  "include": ["next-env.d.ts", "**/*.tsx", "**/*.ts", ".wesjet/generated"]
  //                                                ^^^^^^^^^^^^^^^^^^^^^^^^^^^
}
```

Wesjet generates files in the `.wesjet/generated` directory. This tells JavaScript (or TypeScript) to create an alias of `wesjet/generated` to the generated files directory.
...
import { defineDocumentType, makeSource } from 'wesjet/source-files'
...
import { allPosts } from 'wesjet/generated'
...
Notice that the data was already available to us as `allPosts`, coming from `wesjet/generated`. We used `allPosts` to sort the post in reverse chronological order, and then sent the posts to the home page as props.
...
Notice again that we're importing data from `wesjet/generated`. This is the beauty of Wesjet. It has already loaded and shaped our date and keeps the logic in `getStaticPaths()` and `getStaticProps()` nice and simple.

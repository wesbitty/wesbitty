    ...
    let actions: Action[] = [
      {
        id: '0-homepage',
        name: 'Homepage',
        keywords: 'Wesjet Home Start Index Overview Features Intro',
        section: 'Home',
        perform: () => router.push('/'),
      },
      {
        ...
        keywords: 'Wesjet Github Git Repository Repo Code Examples',
        section: 'External',
        perform: () =>
          window.open('https://github.com/wesbitty/wesjet', '_ blank'),
      },
      {
        id: '3-discord',
          ...
          <KBarAnimator className="w-full max-w-xl">
            <Card>
              <div className="flex items-center space-x-4 p-4">
                ...
                <KBarSearch className="h-8 w-full bg-transparent text-slate-1200 placeholder-slate-1200 focus:outline-none dark:text-slate-100 dark:placeholder-slate-100" />
                <Label text="ESC" />
              </div>
              <RenderResults />
            </Card>
...
const RenderResults = () => {
  ...
  if (results.length) {
      ...
      <KBarResults
        ...
        onRender={({ item, active }) => (
          <div>
            {typeof item === 'string' ? (
              <div className="pt-3">
                <div className="block border-t border-gray-100 px-4 pt-6 pb-2 text-xs uppercase text-slate-1100 dark:border-gray-800 dark:text-slate-500">
                  {item}
                </div>
              </div>
            ) : (
              <div
                className={`block cursor-pointer px-4 py-2 text-slate-1200 dark:text-slate-300 ${
                  active ? 'bg-gray-100 dark:bg-gray-800' : 'bg-transparent'
                }`}
              >
                {item.subtitle && (
                  <div className="text-xs text-slate-1100 dark:text-slate-500">
                    {item.subtitle}
                  </div>
                )}
                <div>{item.name}</div>
    ...
    return (
      <div className="block border-t border-gray-100 px-4 py-8 text-center text-slate-1100 dark:border-gray-800 dark:text-slate-1200">
